...
function SheetContent({ className, children, pushed = false, showCloseButton = true, style, ...props }: React.ComponentProps<typeof KumoDialog> & { pushed?: boolean; showCloseButton?: boolean }) {
	return <KumoDialog data-slot="sheet-content" data-pushed={pushed || undefined} size="lg" style={{ transitionProperty: "transform, translate, scale", transitionDuration: "240ms", transitionTimingFunction: "cubic-bezier(0.32, 0.72, 0, 1)", ...style }} className={cn("!fixed !z-[100] !bottom-2 !left-2 !right-2 !flex !h-[min(80dvh,44rem)] !w-[calc(100vw-1rem)] !max-w-none !flex-col !overflow-hidden !p-0 data-[starting-style]:!scale-100 data-[starting-style]:!opacity-100 data-[ending-style]:!scale-100 data-[ending-style]:!opacity-100 sm:!inset-y-3 sm:!left-auto sm:!right-3 sm:!h-[calc(100dvh-1.5rem)] sm:!w-[min(var(--sheet-width,32rem),calc(100vw-1.5rem))]", className)} {...props}>
		{children}
		{showCloseButton ? (
